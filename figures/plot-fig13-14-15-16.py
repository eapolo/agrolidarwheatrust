...
correlation_columns = ["Prop50", "Prop60","Prop70","Prop80","Prop90" ,"Biomass", "Evol", "Parea", "type_wheat", "variety","Inoculate"]
df_selected = df[correlation_columns].copy()

# Convert to numeric where necessary
for col in ["Prop50", "Prop60","Prop70","Prop80","Prop90","Biomass", "Evol", "Parea"]:
...
df_non_inoculated = df_selected[df_selected["Inoculate"] == 1] #Change this to 0 for non-inoculate
...
for wheat in wheat_types:
    ...
    cultivars = sorted(df_non_inoculated[df_non_inoculated["type_wheat"] == wheat]["variety"].unique())  # Sorted alphabetically
    ...
    for idx, cultivar in enumerate(cultivars):
        ...
        corr_matrix = subset[["Prop50", "Prop60","Prop70","Prop80","Prop90" ,"Biomass", "Evol", "Parea"]].corr(method="pearson")
        ...
        sns.heatmap(
            ...
            vmin=vmin, vmax=vmax, linewidths=1, annot_kws={"size": 10},linecolor='white', ax=ax
        )
        ax.set_title(f"{cultivar}")
        ax.set_xticklabels(ax.get_xticklabels(), fontsize=12, fontfamily="Palatino Linotype", rotation=90)
